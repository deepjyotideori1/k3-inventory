...
const DashboardSelector = () => {
  const { user, canAccessHRMS, canAccessInventory } = useAuth();
  ...
  useEffect(() => {
    if (!loading) {
      if (canAccessHRMS && !canAccessInventory) {
        selectDashboard('hrms');
      } else if (canAccessInventory && !canAccessHRMS) {
        selectDashboard('inventory');
      }
    }
  }, [loading, canAccessHRMS, canAccessInventory]);
  ...
  // If only one dashboard, show loading while auto-redirecting
  if ((canAccessHRMS && !canAccessInventory) || (canAccessInventory && !canAccessHRMS)) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <Loader2 className="w-8 h-8 animate-spin text-green-700" />
